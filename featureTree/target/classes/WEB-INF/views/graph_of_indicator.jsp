...
<title>Statistics of Feature</title>
...
<body id="pgbody" onLoad="initDefaultGraphData()">
	<div align="center">
		<font id="tVisualAnalytics" size="4" face="Verdana"color"#800080">
			Visual Analytics of Product Features |<a  id="tHomeLink"href="/featureTree"> Home</a>
		</font>
	</div>
	<div id="container" style="height: 550px; width: 1300px">
		<div id="menu" style="background-color: #F3F3F3; height: 550px; width: 240px; float: left;">
			<div id="menu1_1" style="overflow: auto; height: 275px; width: 240px; float: left; text-align: left; border: gray 1px solid;">
			 	<b><font size="2" face="vardana">Feature Tree1:</font></b><br>
					<c:set var="f_len" value="${features.size()}" /> 
					<a href="#" id="node1_0" onclick="getFeatureId1(0,'node1_0',${f_len})"><font id="fTreeProduct1" size="2" face="times new roman">Product</font></a>
					<c:if test="${not empty features}">
						<c:forEach var="feature" items="${features}" varStatus="features">
	 						<li><a href="#" id="node1_${feature.id}" onclick="getFeatureId1(${feature.id},'node1_${feature.id}',${f_len})">
								<font id="fTreeFeature1_${feature.id}" size="1" face="times new roman">${feature.name}</font></a>
							</li>
						</c:forEach>
					</c:if>
			...
			<div id="menu1_2" style="overflow: auto; height: 275px; width: 240px; float: left; text-align: left; border: gray 1px solid;">
				<b><font size="2" face="vardana">Feature Tree2:</font></b><br>
				<a href="#" id="node2_0" onclick="getFeatureId2(0,'node2_0',${f_len})"><font id="fTreeProduct2" size="2" face="times new roman">Product</font></a>
				<c:if test="${not empty features}">
					<c:forEach var="feature" items="${features}" varStatus="features">
						<li><a href="#" id="node2_${feature.id}" onclick="getFeatureId2(${feature.id},'node2_${feature.id}',${f_len})">
							<font id="fTreeFeature2_${feature.id}" size="1" face="times new roman">${feature.name}</font></a>
						</li>
					</c:forEach>
				</c:if>
			</div>
...
<script type="text/javascript">
	...
	$('#pgbody').click(function (event) 
	{
		...
		if((admin_status == 1) && (clickedEvent != '') && (clickedEvent != 'pgbody')){
			var popupWindow = window.open("http://localhost:8080/featureTree/mapGuielementPM?eventId="+clickedEvent, "popupWindow", "height=550, width=780");
		}
		...
		function showPosition(position)
		{
			latitute=position.coords.latitude;
		   	...
		   	geocoder.geocode({'latLng': latlng}, function(results, status){
				...
				{
					$.get('http://localhost:8080/featureTree/addGuielementvisitPM', {eventvalue:clickedEvent, sessionId:sessionId, visitorAddress:visitorAddress, ipaddress:ipaddress}, 
					function(data)
					{
						//alert("Respn");
						/*if(data == 1)
